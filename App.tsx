...
// Helper para convertir objetos a array para Sheets
...
export default function App() {
    ...
    const handleLogin = () => window.gapi.auth2.getAuthInstance().signIn();
    const handleLogout = () => window.gapi.auth2.getAuthInstance().signOut();
    ...
    const handleSaveToDrive = async () => {
        if (!isSignedIn) return alert("Debes iniciar sesión.");
        setIsSaving(true);
        const timestamp = new Date().toISOString();

        try {
            // 1. PREPARAR DATOS (ORDEN DE COLUMNAS DEBE COINCIDIR CON SHEETS)
            
            // Tabla: Maestro_Cotizaciones
            ...
            alert("Error al guardar en Drive. Revisa la consola y los permisos.");
        }
    };
    ...
    const handleNewQuote = () => {
        if(window.confirm("¿Nueva cotización? Se perderán cambios no guardados.")) {
